    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const userResponse = await fetch(`http://localhost:8081/profile/visitante/${userId}`, {
                    credentials: 'include'
                });
                ...
                if (userData.user.IMAGE) {
                    try {
                        const imgResponse = await fetch(`http://localhost:8081/pictures/${userData.user.IMAGE}`, {
                            credentials: 'include'
                        });
                ...
                const postsResponse = await fetch(`http://localhost:8081/posts/user/${userId}`);
            ...
            <div className="max-w-4xl mx-auto p-4">
                ...
                <div className="mb-6">
                    ...
                    {userData.musician?.COMENTARIO ? (
                        <p className="text-sm whitespace-pre-line">{userData.musician.COMENTARIO}</p>
                    ) : (
                    ...
                    {posts.length === 0 ? (
                        <div className="col-span-3 flex flex-col items-center justify-center py-10">
                            ...
                            <p className="text-gray-400 text-center max-w-md">
                                Quando este usuário compartilhar fotos e vídeos, eles aparecerão aqui.
                            </p>
                        </div>
                    ) : (
                        posts.map(post => (
